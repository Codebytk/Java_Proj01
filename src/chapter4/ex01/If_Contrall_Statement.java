...
public class If_Contrall_Statement {
	...
	public static void main(String[] args) {
		...
		//1. type1 : if (조건) {실행문 : 조건이 거짓일때 실행;}
			//{} 를 생략 가능한데, 갱략될경우 1개만 가능
		if (value1 < 3) {     //조건이 참일때 실행블락
		System.out.println("실행2"); //조건이 거짓이므로 실행되지 않음
		}
		System.out.println("====={조건이 참일떄 실행문} , {} 생략가능 ======0");
		    //if 조건에서 실행문이 하나일때 {}생략 가능 실행문 하나만 해당됨
		if (value1 < 3)
			System.out.println("실행3");  //실행문 1개만 {} 가 생략되어서 처리될 수 있다.
		    System.out.println("실행4"); 
		    System.out.println("================================");
		//2. type2 : if (조건){조건이 참일때 실행;} else { 조건이 거짓일때 실행;}
		    // 삼항 연산자로 변환 
		boolean bool1 = true;
		boolean bool2 = false;
		...
		//if (조건1) {조건1 이 참일떄 실행}
		//else if (조건2){조건2가 참일때실행}
		//else if (조건)3 {조건3이 참일때실행}
		// ...
		// ...
		//else {위 조건이 존재하지 않는 경우 실행}
		int value3 = 90;
		if (value3>= 90) {
			System.out.println("A학점");
		}else if (value3>=80) {
			System.out.println("B학점");  //if 문을 빠져나온다. 실행 후 탈출
		}else if (value3>=70) {
		...
		if (value3 > 60 && value3 <= 70) {
			...
			System.out.println("B학점");
		}else if (90<=value3&&value3<=100) {
			System.out.println("A학점");
		}else if (0<=value3&&value3<=60) {
			System.out.println("F학점");
		}else {
