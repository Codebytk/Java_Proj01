...
public class Array_Definition {
	...
	public static void main(String[] args) {
		/*
		 ...
		 배열 - 하나의 배열 변수에 많은 값을 저장 할 수 있다.
		 	- 배열의 특징 : 1. 배열변수의 동일한 자료형의 값만 저장할 수 있다.
		 				 2. 배열은 방의 갯수가 정의되면 수정 불가능
		 	-배열은 초기값을 할당하지 않아도 강제로 초기값이 할당됨
		...
		//변수 선언후 초기값을 설정하지않고 사용하면 오류발생
		int c;
		int d;
		//int sum c+d;    //오류발생 : c,d변수에 초기값을 넣지 않고 해용했으므로 오류
		
		System.out.println("==배열=======================");
		// 1. 배열 선언 1방법
		int[] array1= new int[3];
		//int[] : 정수 값만 배열에 저장가능
		//array1 : 배열 변수 , array1 변수명은 stack 공간에 저장이 되어있고, array1 에 heap영역에 주소값이 저장
		//new : Heap 영역에 값을저장하라.
		//int[3]: heap 영역에 저장할 샛수를 지정
		//			index :반의 번호 ,0,1,2,................
			
		//배열에 값할당
		array1[0]=55; //array 의 Heap 영역의 주소 0번째 55를 넣어라
		array1[1]=66;
		array1[2]=77;
		//배열의 값 출력
		...
		int[] array2;		//배열 변수 선언
		array2=new int [5];	//배열변수의 들어갈 방의 갯수를 저장, index 0~4
	//array2 배열 변수에 값을 넣고 출력
		...
		System.out.println("0번째값 출력"+array2[0]);
		System.out.println("1번째값 출력"+array2[1]);
		System.out.println("2번째값 출력"+array2[2]);
		System.out.println("3번째값 출력"+array2[3]);
		System.out.println("4번째값 출력"+array2[4]);
		...
		// System.out.println(array3[4]); 없는 값이기때문에 오류2
		
		//선언과 방번호를 분리해서 
		int array4[];
		array4=new int [5];
		
		System.out.println("==============배열 :1 선언방법============");
		
		//배열선언과 동시에 값 할당
		int[] array5 = new int[] {10,20,30,40,50};
		//int[] array6 = new int[5] {10,20,30,40,50};
			// 오류 발생 : 선언과 동시에 배열 변수에 값을 할당 할때 방크기르 저장하면 오류 발생
		...
		//System.out.println(array7[6]); //컴파일 단계에서 오류 가 안나지만 실행시 오류 
		//ArrayIndexOutOfBoundsException :배열의 방번호를 넘어가면 발생
		...
		//오류 발생 배열선언과 동시에 값할당은 선언과 값할당을 분리할수없다
