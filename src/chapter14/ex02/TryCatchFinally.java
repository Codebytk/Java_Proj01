...
public class TryCatchFinally {
	...
	public static void main(String[] args) {
		/*	예외를 처리하는 방법 
		 		1. 자신이 직접 예외를 처리하는 방법 : try catch finally 를 사용해서 처리
		...
		try {
			...
			//try 블락에서 예외가 발생되지 않으면 catch block 은 실행되지 않는다.
			System.out.println(3/0); 	// 실행예외 : (ArithmeticException)
		} catch (ArithmeticException e) {
			//try 블랏에서 Arithmetic Exception 이 발생이 되었을때 작동
			//try 블랏에서 Arithmetic Exception 이 발생이 되지 않았을 때는 작동되지 않는다.
			
			System.out.println("try 블락에서 오류가 발생");
		}
		System.out.println("=================================");
